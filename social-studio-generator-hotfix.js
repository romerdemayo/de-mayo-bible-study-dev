/* De Mayo Bible Studies — premium generator controls hotfix */
(function(){
'use strict';
...
function run(kind){
 ...
 if(btn){btn.click();setTimeout(()=>{document.querySelector('#dmSocialDesigner')?.scrollIntoView({behavior:'smooth',block:'start'});},120);}
 else window.toast?.('Generator is still loading. Please try again.');
}
function mount(){
 ...
 if(!panel){
  ...
  panel.className='dm-premium-generator-hotfix';
 ...
 if(!$('#dmPremiumGeneratorHotfixStyles')){
  const s=document.createElement('style');s.id='dmPremiumGeneratorHotfixStyles';s.textContent=`
