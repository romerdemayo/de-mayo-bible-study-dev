...
function reelWrapText(ctx,text,maxWidth){const words=String(text||'').split(/\s+/),lines=[];let line='';for(const word of words){const test=line?line+' '+word:word;if(ctx.measureText(test).width>maxWidth&&line){lines.push(line);line=word}else line=test}if(line)lines.push(line);return lines}
function reelDrawExportFrame(ctx,w,h,scene,progress){
 ...
 const fade=Math.min(1,progress/.12,(1-progress)/.12);ctx.globalAlpha=Math.max(.05,fade);ctx.textAlign='center';ctx.fillStyle='rgba(255,255,255,.82)';ctx.font='700 34px system-ui, sans-serif';ctx.fillText(sc.k,w/2,230);
 ctx.fillStyle='#ffffff';ctx.font='700 62px system-ui, sans-serif';const lines=reelWrapText(ctx,sc.t,w-150);const lh=82;let y=h/2-(lines.length*lh)/2;for(const line of lines.slice(0,9)){ctx.fillText(line,w/2,y);y+=lh}
 if(sc.s){ctx.fillStyle='rgba(255,255,255,.9)';ctx.font='500 38px system-ui, sans-serif';const sub=reelWrapText(ctx,sc.s,w-170);for(const line of sub.slice(0,3)){ctx.fillText(line,w/2,y+30);y+=52}}
 ctx.fillStyle='rgba(255,255,255,.85)';ctx.font='600 30px system-ui, sans-serif';ctx.fillText('De Mayo Bible Studies',w/2,h-115);ctx.globalAlpha=1;
}
