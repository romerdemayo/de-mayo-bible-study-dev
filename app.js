...
const navGroups=[
 ...
 ['Create',[['creator','✨ Quick Create'],['notes','📝 Bible Study Creator'],['prayer','🙏 Prayer Creator'],['sermon','🎤 Sermon Studio'],['kids','🧒 Kids Ministry Studio'],['socialstudio','🎬 Social Studio']]],
...
let reelState={topic:'hope',duration:30,voice:'female',sound:'soft',scene:0,timer:null,audio:null,recording:false,recordStream:null,recorder:null};
...
function reelNarrationText(){return [$('#reelHook')?.value,$('#reelReference')?.value,$('#reelVerse')?.value,$('#reelReflection')?.value,'Let us pray.',$('#reelPrayer')?.value].filter(Boolean).join('. ')}
function reelSetExportStatus(message,state='working'){const box=$('#reelExportStatus');if(!box)return;box.hidden=false;box.className=`reel-export-status ${state}`;box.innerHTML=message}
function reelStopRecording(){if(reelState.recorder&&reelState.recorder.state!=='inactive')reelState.recorder.stop();if(reelState.recordStream){reelState.recordStream.getTracks().forEach(t=>t.stop());reelState.recordStream=null}reelState.recording=false;const b=$('#reelExport');if(b){b.disabled=false;b.textContent='🎥 Record full reel with audio'}}
async function reelExportWithAudio(){
 if(reelState.recording)return reelStopRecording();
 if(!navigator.mediaDevices?.getDisplayMedia||!window.MediaRecorder)return reelSetExportStatus('⚠️ Full audio recording needs desktop Chrome or Microsoft Edge.','error');
 let stream;
 try{
  reelSetExportStatus('1️⃣ In the sharing window, choose <b>This Tab</b> and turn on <b>Share tab audio</b>.','working');
  stream=await navigator.mediaDevices.getDisplayMedia({video:{frameRate:30},audio:true,preferCurrentTab:true,selfBrowserSurface:'include',surfaceSwitching:'exclude'});
 }catch(e){reelSetExportStatus('Recording cancelled. Nothing was saved.','error');return}
 const audioTracks=stream.getAudioTracks();
 if(!audioTracks.length){stream.getTracks().forEach(t=>t.stop());reelSetExportStatus('⚠️ No tab audio was shared. Try again and enable <b>Share tab audio</b>.','error');return}
 reelState.recording=true;reelState.recordStream=stream;
 const chunks=[];let mime='video/webm';
 for(const type of ['video/webm;codecs=vp9,opus','video/webm;codecs=vp8,opus','video/webm'])if(MediaRecorder.isTypeSupported(type)){mime=type;break}
 const rec=new MediaRecorder(stream,{mimeType:mime,videoBitsPerSecond:5000000,audioBitsPerSecond:128000});reelState.recorder=rec;
 rec.ondataavailable=e=>{if(e.data?.size)chunks.push(e.data)};
 rec.onerror=()=>reelSetExportStatus('The recording stopped because the browser reported an error.','error');
 rec.onstop=()=>{if(chunks.length){const blob=new Blob(chunks,{type:mime});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`de-mayo-bible-reel-${Date.now()}.webm`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),5000);reelSetExportStatus('✅ Reel downloaded with the tab audio as a WebM video. Upload it directly where supported, or convert it to MP4 in CapCut/VLC.','ready');recordInsight('feature','reel-webm-export')}else reelSetExportStatus('No video data was recorded. Please try again.','error');reelStopRecording();document.body.classList.remove('reel-recording-mode');if(document.fullscreenElement)document.exitFullscreen().catch(()=>{})};
 stream.getVideoTracks()[0].addEventListener('ended',()=>{if(rec.state!=='inactive')rec.stop()});
 const phone=$('#reelPhone');document.body.classList.add('reel-recording-mode');try{await phone?.requestFullscreen?.()}catch(e){}
 const b=$('#reelExport');if(b){b.disabled=false;b.textContent='⏹ Stop & download'};
 reelSetExportStatus('🔴 Recording now. Keep this tab visible. The reel will stop automatically when the selected duration ends.','recording');
 rec.start(1000);reelStartPreview();reelSoundPreview();setTimeout(()=>reelSpeak(),500);
 const duration=Math.max(15,+($('#reelDuration')?.value||30));
 setTimeout(()=>{reelStopPreview(false);reelStopSound();speechSynthesis?.cancel?.();if(rec.state!=='inactive')rec.stop()},duration*1000+1200);
}
function reelStudioHTML(){return `<section class="card reel-studio"><div class="section-heading compact"><div><span class="eyebrow">BETA · REELS & SHORTS</span><h3>🎬 ${ui('Bible Reel Creator','Bible Reel Creator')}</h3><p>${ui('Create a short animated Scripture reel with voice reading and ambient sound preview.','Gumawa ng maikling animated Scripture reel na may voice reading at ambient sound preview.')}</p></div></div><div class="reel-layout"><div class="reel-controls"><div class="form-grid"><label>${ui('Topic','Paksa')}<select id="reelTopic">${Object.entries(REEL_TOPICS).map(([k,v])=>`<option value="${k}">${v.label}</option>`).join('')}</select></label><label>${ui('Duration','Tagal')}<select id="reelDuration"><option value="15">15 seconds</option><option value="30" selected>30 seconds</option><option value="45">45 seconds</option><option value="60">60 seconds</option></select></label><label>${ui('Narrator','Tagabasa')}<select id="reelVoice"><option value="female">Female · Gentle</option><option value="male">Male · Warm</option></select></label><label>${ui('Ambient sound','Tunog')}<select id="reelAmbient"><option value="soft">Soft worship tone</option><option value="rain">Gentle rain</option><option value="ocean">Ocean ambience</option></select></label></div><button class="primary wide" id="reelGenerate">✨ ${ui('Generate reel script','Gumawa ng reel script')}</button><label>${ui('Opening hook','Panimulang hook')}<textarea id="reelHook" rows="2"></textarea></label><div class="form-grid"><label>${ui('Verse reference','Verse reference')}<input id="reelReference"></label></div><label>${ui('Verse','Talata')}<textarea id="reelVerse" rows="4"></textarea></label><label>${ui('Reflection','Reflection')}<textarea id="reelReflection" rows="3"></textarea></label><label>${ui('Prayer','Panalangin')}<textarea id="reelPrayer" rows="3"></textarea></label><div class="reel-actions"><button class="primary" id="reelPlay">▶ Preview animation</button><button class="ghost" id="reelVoicePreview">🎙 Read the verse</button><button class="ghost" id="reelSound">🔊 Preview sound</button><button class="ghost" id="reelCopy">📋 Copy script</button><button class="primary reel-export-button" id="reelExport">🎥 Record full reel with audio</button></div><div id="reelExportStatus" class="reel-export-status" hidden></div><p class="privacy-note">${ui('Free export: on desktop Chrome or Edge, choose This Tab and enable Share tab audio. The app records the animation, device voice and ambient sound into a WebM video. No Google service, API key or payment is used.','Libreng export: sa desktop Chrome o Edge, piliin ang This Tab at i-enable ang Share tab audio. Ire-record ng app ang animation, device voice at ambient sound bilang WebM video. Walang Google service, API key o bayad.')}</p></div><div class="reel-preview-column"><div class="reel-phone" id="reelPhone"></div><small>${ui('9:16 phone preview','9:16 phone preview')}</small></div></div></section>`}
function bindReelStudio(){const topic=$('#reelTopic');if(!topic)return;topic.value=reelState.topic;$('#reelDuration').value=reelState.duration;$('#reelVoice').value=reelState.voice;$('#reelAmbient').value=reelState.sound;reelSyncFields();topic.onchange=e=>{reelState.topic=e.target.value;reelSyncFields()};$('#reelDuration').onchange=e=>reelState.duration=+e.target.value;$('#reelVoice').onchange=e=>reelState.voice=e.target.value;$('#reelAmbient').onchange=e=>reelState.sound=e.target.value;$('#reelGenerate').onclick=()=>{reelSyncFields();toast(ui('Reel script generated','Nagawa ang reel script'))};$('#reelPlay').onclick=()=>reelState.timer?reelStopPreview():reelStartPreview();$('#reelVoicePreview').onclick=reelSpeak;$('#reelSound').onclick=()=>reelState.audio?reelStopSound():reelSoundPreview();$('#reelCopy').onclick=reelCopyScript;$('#reelExport').onclick=reelExportWithAudio;['Hook','Reference','Verse','Reflection','Prayer'].forEach(k=>$('#reel'+k).oninput=()=>reelRenderScene(reelState.scene))}
