/* De Mayo Bible Studies — manual Reel content option v2
...
function parseOnePaste(raw){
 ...
 if(!verse||!reflection||!prayer){
   ...
   if(reference){
     ...
     if(!verse){
       const candidate=refIndex>=0?blocks[refIndex].replace(reference,'').replace(/^(?:verse|scripture)\s*[:\-]?\s*/i,''):blocks[0];
       verse=clean(candidate);
     }
...
function loadContent({reference,verse,reflection,prayer,title}){
 ...
 const finalTitle=clean(title)||titleFrom(reference,reflection);
 api.setGeneratedContent({title:finalTitle,label:'My Content',contentType:current.contentType||'devotional',reference,verse,reflection,prayer,caption:`${reflection} What part of this message speaks to you today?`,hashtags:'#BibleVerse #ChristianEncouragement #Faith #DeMayoBibleStudies',language:$('#dmReelLanguage')?.value||'English',source:'Manual'});
 setTimeout(()=>{window.DM_REEL_VOICEOVER_DURATION_FIX?.apply?.();document.dispatchEvent(new CustomEvent('dm-reel-generated'));},80);
 status(`✅ Your content is in the Reel as “${finalTitle}”. You can now preview it, record your voice-over and create the MP4.`,'success');
...
function clear(){['dmManualAllInOne','dmManualTitle','dmManualReference','dmManualVerse','dmManualReflection','dmManualPrayer'].forEach(id=>{const el=$('#'+id);if(el)el.value='';});const box=$('#dmManualReelStatus');if(box)box.hidden=true;}
function install(){
...
window.DM_REEL_MANUAL_CONTENT={install,applyOnePaste,applySeparate,useCurrent,clear,parseOnePaste,titleFrom};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
})();