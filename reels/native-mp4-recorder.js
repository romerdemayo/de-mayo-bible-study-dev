...
function draw(ctx,s,w,h,p){const g=ctx.createLinearGradient(0,0,w,h);g.addColorStop(0,s.r1);g.addColorStop(1,s.r2);ctx.fillStyle=g;ctx.fillRect(0,0,w,h);ctx.globalAlpha=.12;ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(w*.2,h*.24,w*.18,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(w*.79,h*.68,w*.25,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;ctx.textAlign='center';ctx.fillStyle=s.accent;ctx.font=`800 ${Math.round(w*.032)}px system-ui`;ctx.fillText(s.kicker.toUpperCase(),w/2,h*.205);const f=fit(ctx,s.message,w*.76,h*.38,Math.round(w*.09),Math.round(w*.045));ctx.fillStyle='#fff';ctx.font=`800 ${f.size}px system-ui`;let y=h*.45-(f.lines.length*f.lh)/2;for(const line of f.lines){ctx.fillText(line,w/2,y);y+=f.lh;}if(s.ref){ctx.fillStyle=s.accent;ctx.font=`800 ${Math.round(w*.047)}px system-ui`;ctx.fillText(s.ref,w/2,h*.66);}ctx.fillStyle='#fff';ctx.font=`800 ${Math.round(w*.041)}px system-ui`;ctx.fillText('✝  De Mayo Bible Studies',w/2,h*.79);ctx.font=`500 ${Math.round(w*.025)}px system-ui`;ctx.fillText('Grow in faith · Walk in truth',w/2,h*.82);ctx.fillStyle='rgba(255,255,255,.25)';ctx.fillRect(w*.09,h*.855,w*.82,5);ctx.fillStyle=s.accent;ctx.fillRect(w*.09,h*.855,w*.82*Math.max(0,Math.min(1,p)),5);}
