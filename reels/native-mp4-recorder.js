...
async function create(options={}){if(busy)return;const forceVoiceOnly=options?.voiceOnly===true;if(forceVoiceOnly&&!window.DM_REEL_VOICEOVER_BLOB?.size){status('Record your voice first, then create the Voice Only MP4.','error');return;}busy=true;clearOldFailure();const btn=document.querySelector('#dmNativeMp4');const silent=forceVoiceOnly||silentSelected(),hasVoice=!!window.DM_REEL_VOICEOVER_BLOB?.size;if(btn){btn.disabled=true;btn.textContent='⏳ Starting HD MP4…';}status(`Starting animated 1080 × 1920 MP4${hasVoice?' with your voice':''}… Keep Safari open.`,'info');await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));let music=null;try{const scenes=await collect();if(!scenes.length)throw new Error('Reel preview is not ready');const selected=Number(document.querySelector('#dmDuration')?.value||15);const duration=hasVoice?Math.max(15,Math.min(120,Math.ceil(Number(window.DM_REEL_VOICEOVER_DURATION||15)+1))):IOS?15:Math.max(9,Math.min(30,selected));const w=EXPORT_WIDTH,h=EXPORT_HEIGHT,fps=EXPORT_FPS;const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d',{alpha:false,desynchronized:true});const videoStream=canvas.captureStream(fps);let restoreTrack=null;if(forceVoiceOnly){const sound=document.querySelector('#dmExportMusic');if(sound){restoreTrack=sound.value;sound.value='none';}}music=await makeMusic(duration);if(forceVoiceOnly&&restoreTrack!==null){const sound=document.querySelector('#dmExportMusic');if(sound)sound.value=restoreTrack;}const tracks=[...videoStream.getVideoTracks(),...(music?music.destination.stream.getAudioTracks():[])];const stream=new MediaStream(tracks);const mime=MP4_TYPES.find(t=>MediaRecorder.isTypeSupported(t));const rec=new MediaRecorder(stream,{mimeType:mime,videoBitsPerSecond:EXPORT_VIDEO_BITRATE,audioBitsPerSecond:128000});const chunks=[];rec.ondataavailable=e=>e.data.size&&chunks.push(e.data);const done=new Promise((resolve,reject)=>{rec.onstop=resolve;rec.onerror=e=>reject(e.error||new Error('Recording failed'));});rec.start(500);const total=duration*1000,start=performance.now(),timing=sceneTiming(scenes,duration,hasVoice);await new Promise(resolve=>{function frame(now){const p=Math.min(1,(now-start)/total),elapsed=(now-start)/1000,moment=sceneMoment(elapsed,timing),idx=moment.index,sp=moment.progress;draw(ctx,scenes[idx]||scenes[0],w,h,sp,idx===scenes.length-1);const pct=Math.round(p*100);if(btn)btn.textContent=`⏳ Creating HD MP4 ${pct}%`;status(`Creating animated 1080p MP4 ${pct}%… Keep Safari open.`);if(p<1)requestAnimationFrame(frame);else resolve();}requestAnimationFrame(frame);});rec.stop();await done;const blob=new Blob(chunks,{type:'video/mp4'});if(!blob.size)throw new Error('Empty MP4 file');if(window.DM_MP4_RESULT_READY)window.DM_MP4_RESULT_READY(blob);else URL.createObjectURL(blob);clearOldFailure();status(`Animated 1080 × 1920 MP4${music?.hasVoice?' with voice-over':''} ready (${(blob.size/1024/1024).toFixed(1)} MB). Tap Share MP4, then Save Video.`,'success');}
...
function installSoundControls(actions){
  ...
  card.innerHTML='<h3>🎵 Reel sound</h3><div class="dm-form-grid"><label>Sound option<select id="dmExportMusic"><option value="gentle">Gentle ambient</option><option value="hope">Hopeful instrumental</option><option value="prayer">Prayer atmosphere</option><option value="none">Voice only — no background music</option></select></label><label>Music volume<input id="dmExportMusicVolume" type="range" min="0" max="60" value="28"><span id="dmExportMusicVolumeValue">28%</span></label></div><p class="small-note">Choose <b>Voice only — no background music</b> to keep your recorded voice and remove the background music.</p>';
...
window.DM_CREATE_VOICE_ONLY_MP4=()=>create({voiceOnly:true});
