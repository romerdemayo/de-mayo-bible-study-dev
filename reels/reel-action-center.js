/* De Mayo Bible Studies — Reel Action Center v5 — complete collapsible workflow */
(function(){
'use strict';
...
function createBody(){
 const type=$('[data-dm-type].active')?.dataset.dmType||'verse';
 return '<div class="dm-content-choice"><button data-type-proxy="verse" class="'+(type==='verse'?'active':'')+'">📖<b>Bible Verse</b><small>Verse + Message + Prayer</small></button><button data-type-proxy="motivation">🔥<b>Christian Motivation</b><small>Encouragement</small></button><button data-type-proxy="devotional" class="'+(type==='devotional'?'active':'')+'">❤️<b>Devotional</b><small>Reflection & Prayer</small></button><button data-open-weekly>📅<b>Weekly Reel</b><small>Series Content</small></button><button data-open-sound>🎵<b>Reel Sounds</b><small>Music & Audio</small></button></div><div class="dm-dash-fields">'+selectProxy('Theme','dmTheme')+selectProxy('Template','dmTemplate')+selectProxy('Length','dmDuration')+selectProxy('Language','dmReelLanguage')+'</div><div class="dm-dash-status"><span>● <b>Gemini</b> <i id="dmDashGemini">Ready</i></span><span id="dmDashPosted"></span></div><div class="dm-action-grid dm-create-actions">'+actionButton('dmRegenerate','✨','Generate','Create Your Reel')+actionButton('dmReelSurprise','🎲','Surprise Me','Random Theme')+actionButton('dmManualOnePaste','✎','Use My Content','Paste Your Own')+'</div>';
}
...
function wire(){
 ...
 panel.querySelectorAll('[data-type-proxy]').forEach(b=>b.onclick=()=>{const type=b.dataset.typeProxy;if(type==='motivation'){const c=$('#dmReelContentType');if(c){c.value='motivation';c.dispatchEvent(new Event('change',{bubbles:true}));}}else{const target=$('[data-dm-type="'+type+'"]');if(target)target.click();const c=$('#dmReelContentType');if(c&&type==='devotional'){c.value='devotional';c.dispatchEvent(new Event('change',{bubbles:true}));}}setTimeout(build,80);});
