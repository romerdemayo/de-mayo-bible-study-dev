...
function refreshScript(){const area=$('#dmVoiceoverScript');if(area){area.textContent=script();area.scrollTop=0;const count=words(area.textContent).length,estimate=Math.ceil(count/105*60);const info=$('#dmVoiceoverWordCount');if(info)info.textContent=`${count} words • about ${Math.min(90,estimate)} seconds at a calm reading pace`}}
