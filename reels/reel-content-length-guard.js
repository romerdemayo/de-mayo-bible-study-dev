...
function shorten(text,maxWords){const value=clean(text),words=value.split(' ').filter(Boolean);if(words.length<=maxWords)return value;const hard=words.slice(0,maxWords).join(' '),minChars=Math.floor(hard.length*.62),matches=[...hard.matchAll(/[.!?](?=\s|$)/g)],last=matches.map(m=>m.index+1).filter(i=>i>=minChars).pop();return last?hard.slice(0,last).trim():hard.replace(/[,:;\-–—]+$/,'').trim()+'…';}
function shortenPrayer(text,maxWords){const closing='In Jesus’ name, Amen.';let body=prayerClose(text).replace(/\s*In Jesus[’'] name, Amen\.\s*$/i,'').trim();const reserve=closing.split(/\s+/).length;body=shorten(body,Math.max(8,maxWords-reserve)).replace(/…$/,'').replace(/[,.!?;:]+$/,'').trim();return `${body}${body?'. ':''}${closing}`;}
